...
## Goal:
* Use `createRadio()` to build a group of options
* Use `.option()` to add choices
* Use `.changed()` to respond when the user makes a selection
* Use `radio.value()` to get the selected value
